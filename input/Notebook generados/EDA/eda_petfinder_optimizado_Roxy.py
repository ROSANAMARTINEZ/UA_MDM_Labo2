...
# ── Rutas adicionales (v2 optimizado) ─────────────────────────────────────────
PATH_SENTIMENT = BASE / 'input' / 'train_sentiment_features.csv'
PATH_METADATA  = BASE / 'input' / 'train_metadata_features.csv'
...
def load_main():
    ...
    raw    = pd.read_csv(BASE / 'train' / 'train.csv')
    df     = raw.copy()
    ...
    # ── Enriquecer con sentiment y metadata (v2 optimizado) ───────────────────
    df['desc_length'] = raw['Description'].fillna('').apply(len)
    df['desc_has_text'] = (df['desc_length'] > 0).astype(int)
    if PATH_SENTIMENT.exists():
        sent = pd.read_csv(PATH_SENTIMENT)
        df = df.merge(sent[['PetID','sentiment_score','sentiment_magnitude',
                             'n_sentences','language']], on='PetID', how='left')
        df['sentiment_score']     = df['sentiment_score'].fillna(0)
        df['sentiment_magnitude'] = df['sentiment_magnitude'].fillna(0)
        df['n_sentences']         = df['n_sentences'].fillna(0)
        df['language']            = df['language'].fillna('unknown')
    if PATH_METADATA.exists():
        meta = pd.read_csv(PATH_METADATA)
        df = df.merge(meta[['PetID','avg_label_score','n_labels','crop_confidence']],
                      on='PetID', how='left')
        df['avg_label_score'] = df['avg_label_score'].fillna(0)
        df['n_labels']        = df['n_labels'].fillna(0)
        df['crop_confidence'] = df['crop_confidence'].fillna(0)
...
# ── Pestaña Texto & Sentiment ────────────────────────────────────────────────
_df_s = DF.copy() if 'sentiment_score' in DF.columns else pd.DataFrame()

def _sent_tab():
    if _df_s.empty or 'sentiment_score' not in _df_s.columns:
        return html.P('No se encontraron archivos de sentiment.', style={'color': TEXT_MUTED})

    df_s = _df_s.copy()
    df_s['AdoptionLabel'] = df_s['AdoptionSpeed'].map(ADOPTION_LABELS)

    # 1. Sentiment score promedio por clase
    sent_by_class = df_s.groupby('AdoptionLabel')['sentiment_score'].mean().reset_index()
    sent_by_class.columns = ['Clase', 'Sentiment Score promedio']
    fig_sent = px.bar(sent_by_class, x='Clase', y='Sentiment Score promedio',
                      color='Clase', template='plotly_white',
                      color_discrete_sequence=PALETTE,
                      title='Sentiment Score promedio por velocidad de adopción')
    fig_sent.update_layout(**chart_layout(yt='Score (−1 a +1)', showlegend=False))

    # 2. Longitud de descripción por clase
    desc_by_class = df_s.groupby('AdoptionLabel')['desc_length'].mean().reset_index()
    desc_by_class.columns = ['Clase', 'Longitud promedio (chars)']
    fig_desc = px.bar(desc_by_class, x='Clase', y='Longitud promedio (chars)',
                      color='Clase', template='plotly_white',
                      color_discrete_sequence=PALETTE,
                      title='Longitud de descripción promedio por clase')
    fig_desc.update_layout(**chart_layout(yt='Caracteres', showlegend=False))
    ...
    # 4. Calidad de imagen por clase
    fig_img = px.bar(
        df_s.groupby('AdoptionLabel')['avg_label_score'].mean().reset_index(),
        x='AdoptionLabel', y='avg_label_score',
        color='AdoptionLabel', template='plotly_white',
        color_discrete_sequence=PALETTE,
    ...
    # 5. Scatter sentiment vs AdoptionSpeed
    fig_scatter = px.box(df_s, x='AdoptionLabel', y='sentiment_score',
                         color='AdoptionLabel', template='plotly_white',
                         color_discrete_sequence=PALETTE,
                         title='Distribución de Sentiment Score por clase')
    fig_scatter.update_layout(**chart_layout(xt='Clase', yt='Score', showlegend=False))

    return html.Div([
        html.H5('Análisis de Texto, Sentimiento e Imágenes',
                style={'color': TEXT_PRIMARY, 'fontWeight': '600', 'marginBottom': '0.3rem'}),
        html.P('Fuentes: Google Natural Language API (sentiment) + Google Vision API (metadata de imágenes)',
               style={'color': TEXT_MUTED, 'fontSize': '0.8rem', 'marginBottom': '1rem'}),
        dbc.Row([
            dbc.Col([
                dbc.Card(dbc.CardBody([
                    html.P('Sentiment promedio', style={'color': TEXT_MUTED, 'fontSize': '0.8rem', 'margin': 0}),
                    html.H3(f'{_df_s["sentiment_score"].mean():.3f}',
                            style={'color': C_GREEN, 'fontWeight': '700', 'margin': 0}),
                    html.P('Escala −1 (negativo) a +1 (positivo)', style={'color': TEXT_MUTED, 'fontSize': '0.75rem', 'margin': 0}),
                ]), style={'borderTop': f'3px solid {C_GREEN}', 'borderRadius': '12px'}),
            ], md=4),
            dbc.Col([
                dbc.Card(dbc.CardBody([
                    html.P('Longitud promedio de descripción', style={'color': TEXT_MUTED, 'fontSize': '0.8rem', 'margin': 0}),
                    html.H3(f'{int(_df_s["desc_length"].mean())} chars',
                            style={'color': C_BLUE, 'fontWeight': '700', 'margin': 0}),
                    html.P(f'Sin descripción: {(_df_s["desc_length"]==0).sum()} mascotas',
                           style={'color': TEXT_MUTED, 'fontSize': '0.75rem', 'margin': 0}),
                ]), style={'borderTop': f'3px solid {C_BLUE}', 'borderRadius': '12px'}),
            ], md=4),
            dbc.Col([
                dbc.Card(dbc.CardBody([
                    html.P('Calidad de imagen promedio', style={'color': TEXT_MUTED, 'fontSize': '0.8rem', 'margin': 0}),
                    html.H3(f'{_df_s["avg_label_score"].mean():.3f}',
                            style={'color': C_ORANGE, 'fontWeight': '700', 'margin': 0}),
                    html.P('Score Google Vision (0–1)', style={'color': TEXT_MUTED, 'fontSize': '0.75rem', 'margin': 0}),
                ]), style={'borderTop': f'3px solid {C_ORANGE}', 'borderRadius': '12px'}),
            ], md=4),
        ], className='g-3', style={'marginBottom': '1.5rem'}),
        dbc.Row([
        ...
        dbc.Row([
            dbc.Col([dcc.Graph(figure=fig_desc)], md=6),
            dbc.Col([dcc.Graph(figure=fig_lang)], md=6),
        ], className='g-3'),
        dbc.Row([
            dbc.Col([dcc.Graph(figure=fig_img)], md=12),
        ], className='g-3'),
    ])

tab_texto_content = _sent_tab()

print('Iniciando servidor Optimizado en http://localhost:8051')
...
app.title = 'PetFinder — EDA Optimizado (v2)'
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.H4('PetFinder — EDA Optimizado (v2)',
                    style={'color': TEXT_PRIMARY, 'fontWeight': '700',
                           'margin': '0', 'fontSize': '1.3rem'}),
            html.P('Versión optimizada — incluye análisis de texto, sentimiento y metadata de imágenes',
